...
#' @param matching Choice of matching algorithm. The default matching algorithm is undertaken utilizing the minimum cost bipartite matching with the Hungarian algorithm ("bipartite"), users may choose to use Mahalanobis distance matching ("Maha") or nearest neighbor matching with distances determined by logistic regression ("logit").
#' @keywords
#' @export
#' @examples
#' data(cancer)
...
KMSubtractionMatch=function(df_overall,
  ...
  Parameters=data.frame(rbind('Size of overall cohort'=nrow(df_overall),
              ...
              'Proportion of subgroup (%)'=round(100*nrow(df_subgroup)/nrow(df_overall), 3),
              'Proportion of censorship, overall cohort (%)'=round(prop.table(table(df_overall$status))[[1]]*100, 3),
              'Proportion of censorship, subgroup cohort (%)'=round(prop.table(table(df_subgroup$status))[[1]]*100, 3)
              ))
