...
plot_cox=ggplot(df_analysis, aes(x=TE, fill=matching))+
  ...
  labs(title="Original vs imputed (marginal cox model)",x="ln(HR)", fill="Matching algorithm")+
...
plot_rmst=ggplot(df_analysis, aes(x=RMSTD, fill=matching))+
  ...
  scale_fill_manual(values=c("#999999", "#E69F00", "#56B4E9"))+
  geom_vline(xintercept = 0, linetype="dashed", col="red")+
  labs(title="Original vs imputed (RMST)",x="RMST-Differences", fill="Matching algorithm")+
